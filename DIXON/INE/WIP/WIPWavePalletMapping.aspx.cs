...
{
    public partial class WIPWavePalletMapping : System.Web.UI.Page
    {
        BL_WIPWavePalletMapping blobj = new BL_WIPWavePalletMapping();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Session["usertype"].ToString() != "ADMIN")
                {
                    ...
                    _strRights = CommonHelper.GetRights("WIP Wave Pallet Mapping", (DataTable)Session["USER_RIGHTS"]);
        ...
        protected void txtScanMachineID_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                blobj = new BL_WIPWavePalletMapping();
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows[0][3].ToString().ToUpper().StartsWith("SFG"))
                    {
                        txtSubPCBID.Enabled = true;
                    }
                    else
                    {
                        txtSubPCBID.Enabled = false;
        ...
        protected void ddlModel_Name_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (ddlModel_Name.SelectedIndex > 0)
                {
                    ...
                    txtSubPCBID.Text = string.Empty;
                    txtpcbBarcode.Focus();
                    if (lblmachinetype.Text.ToString().ToUpper().StartsWith("SFG"))
                    {
                        bindSubPCBID();
                        txtpalletid.ReadOnly = true;
                    }
                }
                else
                {
        ...
        protected void txtpcbBarcode_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(txtScanMachineID.Text.Trim()))
                {
                    CommonHelper.ShowMessage("Please Scan Wave Pallet MachineID", msgerror, CommonHelper.MessageType.Error.ToString());
                    txtScanMachineID.Focus();
                    ddlModel_Name.SelectedIndex = 0;
                    txtpcbBarcode.Text = string.Empty;
                    return;
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        txtpalletid.Text = sResult.Split('~')[2];
                        if (string.IsNullOrWhiteSpace(txtpalletid.Text.Trim()))
                        {
                            txtpalletid.ReadOnly = false;
                            txtpalletid.Focus();
                        }
                        else
                        {
        ...
        protected void txtpalletid_TextChanged(object sender, EventArgs e)
        {
            try
            {
                CommonHelper.HideMessage(msginfo, msgsuccess, msgwarning, msgerror);
                ...
                if (string.IsNullOrWhiteSpace(txtpalletid.Text.Trim()))
                {
                    CommonHelper.ShowMessage("Please scan Wave PalletID", msginfo, CommonHelper.MessageType.Info.ToString());
                    txtpalletid.Focus();
                    return;
                }
                blobj = new BL_WIPWavePalletMapping();
                DataTable dt = blobj.ValidateWavePalletID(txtScanMachineID.Text.Trim(), txtpcbBarcode.Text.Trim(),
                    ddlModel_Name.SelectedValue.ToString(), txtpalletid.Text.Trim(), Session["SiteCode"].ToString()
                    , Session["UserID"].ToString(), Session["LineCode"].ToString());
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        CommonHelper.ShowMessage(Message, msgsuccess, CommonHelper.MessageType.Success.ToString());
                        if (lblmachinetype.Text.ToUpper().StartsWith("FG"))
                        {
                            txtpcbBarcode.Focus();
                            txtSubPCBID.Text = string.Empty;
                            txtpalletid.Text = string.Empty;
            ...
            try
            {
                ...
                if (iVerifiedKeyCount == gvModel.Rows.Count)
                {
                    ...
                    DataTable dt = blobj.SaveScanSubPCBID(txtScanMachineID.Text.Trim(), ddlModel_Name.SelectedValue.ToString(),
                        Session["SiteCode"].ToString(), txtpcbBarcode.Text.Trim(), Session["LineCode"].ToString(),
                        ddlModel_Name.SelectedItem.Text.Trim(), Session["UserID"].ToString(), txtpalletid.Text.Trim()
                        , dtRigthsData);
