...
{
    public partial class MSNvsGBComparisonReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["usertype"].ToString() != "ADMIN")
            {
                ...
                _strRights = CommonHelper.GetRights("MSN vs GB Comparison Report", (DataTable)Session["USER_RIGHTS"]);
        ...
        private void BindFGItemCode()
        {
            try
            {
                BL_MSNvsGBComparisonReport blobj;
                blobj = new BL_MSNvsGBComparisonReport();
        ...
        private void ShowGridData()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(txtmsn.Text.Trim()))
                {
                    sMSN = txtmsn.Text.Trim();
                }
                CommonHelper.HideMessage(msginfo, msgsuccess, msgwarning, msgerror);
                BL_MSNvsGBComparisonReport blobj = new BL_MSNvsGBComparisonReport();
                DataTable dtTrackingData = blobj.GetReport(txtFromDate.Text, txtToDate.Text, drpFGItemCode.Text, sMSN);
                if (dtTrackingData.Rows.Count > 0)
                {
                    ...
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/ReportViewer/MSNvsGBComparisonReport.rdlc");
